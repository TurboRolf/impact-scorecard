...
WITH (security_invoker = true) AS
 SELECT c.id,
    ...
    COALESCE(review_stats.avg_marketing_advertising_rating, 0::numeric) AS avg_marketing_advertising_rating,
   ...
   FROM companies c
     LEFT JOIN ( SELECT user_company_stances.company_id,
            count(CASE WHEN user_company_stances.stance = 'recommend'::text THEN 1 END) AS recommend_count,
            count(CASE WHEN user_company_stances.stance = 'neutral'::text THEN 1 END) AS neutral_count,
            count(CASE WHEN user_company_stances.stance = 'discourage'::text THEN 1 END) AS discourage_count
     ...
     LEFT JOIN ( SELECT company_reviews.company_id,
            avg(CASE WHEN company_reviews.category = 'environment'::text THEN company_reviews.rating END) AS avg_environment_rating,
            avg(CASE WHEN company_reviews.category = 'labor_human_rights'::text THEN company_reviews.rating END) AS avg_labor_rating,
            avg(CASE WHEN company_reviews.category = 'ethics_integrity'::text THEN company_reviews.rating END) AS avg_ethics_rating,
            avg(CASE WHEN company_reviews.category = 'politics_lobbying'::text THEN company_reviews.rating END) AS avg_politics_rating,
            avg(CASE WHEN company_reviews.category = 'transparency'::text THEN company_reviews.rating END) AS avg_transparency_rating,
            avg(CASE WHEN company_reviews.category = 'marketing_advertising'::text THEN company_reviews.rating END) AS avg_marketing_advertising_rating,
            avg(CASE WHEN company_reviews.category = 'data_privacy'::text THEN company_reviews.rating END) AS avg_data_privacy_rating,
            avg(CASE WHEN company_reviews.category = 'supply_chain'::text THEN company_reviews.rating END) AS avg_supply_chain_rating,
            avg(company_reviews.rating) AS avg_overall_rating,
...
UPDATE company_reviews SET category = 'marketing_advertising' WHERE category = 'animal_welfare';