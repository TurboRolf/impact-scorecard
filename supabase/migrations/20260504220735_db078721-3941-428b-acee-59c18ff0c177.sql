...
-- Migrate existing review categories to new taxonomy
UPDATE public.company_reviews SET category = 'ethics_integrity' WHERE category = 'ethics';
UPDATE public.company_reviews SET category = 'politics_lobbying' WHERE category = 'politics';
-- Remove any stray 'overall' rows (no longer a valid category)
DELETE FROM public.company_reviews WHERE category = 'overall';

-- Recreate the company_ratings_view with new category columns and computed overall
